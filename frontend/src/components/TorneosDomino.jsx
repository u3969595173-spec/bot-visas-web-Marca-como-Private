...
function TorneosDomino({ admin = false }) {
  ...
  const [formulario, setFormulario] = React.useState({ nombre: '', max_parejas: 30, costo_inscripcion: 0, premio_primero: 0, premio_segundo: 0, premio_tercero: 0 })
  ...
  const crearTorneo = async evento => {
    ...
    try {
      const datos = await request('/api/admin/domino/torneos', { method: 'POST', body: JSON.stringify({ ...formulario, max_parejas: Number(formulario.max_parejas), costo_inscripcion: Number(formulario.costo_inscripcion), premio_primero: Number(formulario.premio_primero), premio_segundo: Number(formulario.premio_segundo), premio_tercero: Number(formulario.premio_tercero) }) })
  ...
  if (!seleccionado) return <main className="domino-page">
    ...
    {admin && <form className="domino-lobby-action domino-torneo-form" onSubmit={crearTorneo}><h2>Crear torneo</h2><div className="domino-torneo-fields"><label>Nombre del torneo<input required placeholder="Ej.: Copa Dominó de Octubre" value={formulario.nombre} onChange={evento => setFormulario({ ...formulario, nombre: evento.target.value })} /></label><label>Máximo de parejas<input type="number" min="4" max="30" step="2" value={formulario.max_parejas} onChange={evento => setFormulario({ ...formulario, max_parejas: evento.target.value })} /></label><label>Inscripción por pareja (fichas)<input type="number" min="0" value={formulario.costo_inscripcion} onChange={evento => setFormulario({ ...formulario, costo_inscripcion: evento.target.value })} /></label><label>Premio 1.º (fichas)<input type="number" min="0" value={formulario.premio_primero} onChange={evento => setFormulario({ ...formulario, premio_primero: evento.target.value })} /></label><label>Premio 2.º (fichas)<input type="number" min="0" value={formulario.premio_segundo} onChange={evento => setFormulario({ ...formulario, premio_segundo: evento.target.value })} /></label><label>Premio 3.º (fichas)<input type="number" min="0" value={formulario.premio_tercero} onChange={evento => setFormulario({ ...formulario, premio_tercero: evento.target.value })} /></label></div><button className="domino-primary" disabled={ocupado}>Crear torneo</button></form>}
    <section className="domino-salas"><div className="domino-section-title"><h2>Torneos</h2><button onClick={cargar}>Actualizar</button></div>{torneos.length ? <div className="domino-salas-grid">{torneos.map(torneo => <article key={torneo.id}><strong>{torneo.nombre}</strong><span>{torneo.parejas_aprobadas}/{torneo.max_parejas} parejas</span><small>Inscripción: {torneo.costo_inscripcion} fichas por pareja</small><small>{torneo.estado}</small><button onClick={() => cargarDetalle(torneo.id)}>Ver torneo</button></article>)}</div> : <p className="domino-empty">No hay torneos creados.</p>}</section>
  </main>
  ...
  return <main className="domino-page">
    <header className="domino-header"><div><p className="domino-eyebrow">Torneo</p><h1>{seleccionado.nombre}</h1><span>{seleccionado.estado} · {seleccionado.rondas_suizas} rondas suizas · Inscripción: {seleccionado.costo_inscripcion} fichas por pareja</span></div><button className="domino-link" onClick={() => { setSeleccionado(null); cargar() }}>Torneos</button></header>
    {error && <p className="domino-error">{error}</p>}
