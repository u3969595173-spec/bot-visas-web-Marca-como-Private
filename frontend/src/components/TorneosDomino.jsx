...
function TorneosDomino({ admin = false }) {
  ...
  return <main className="domino-page">
    ...
    {admin && <section className="domino-ubicacion"><span>Aprueba las parejas completas y genera la siguiente ronda cuando todas las mesas finalicen.</span><div className="domino-admin-actions"><button className="domino-secondary" type="button" onClick={simularBots} disabled={ocupado}>Jugar partida completa con 4 bots</button><button className="domino-primary" onClick={siguienteRonda} disabled={ocupado}>Generar siguiente ronda</button><button className="domino-delete" onClick={() => eliminarTorneo(seleccionado)} disabled={ocupado}>Eliminar torneo</button></div>{simulacion && <p><strong>{simulacion.ganador}</strong> ganó {simulacion.puntuacion?.[0]} - {simulacion.puntuacion?.[1]} en {simulacion.manos_jugadas} manos y {simulacion.turnos} turnos.</p>}</section>}
