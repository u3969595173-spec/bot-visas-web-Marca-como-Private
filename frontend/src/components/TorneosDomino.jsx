...
function TorneosDomino({ admin = false }) {
  ...
  if (!seleccionado) return <main className="domino-page">
    ...
    {admin && <form className="domino-lobby-action domino-torneo-form" onSubmit={crearTorneo}><h2>Crear torneo</h2><div className="domino-torneo-fields"><label>Nombre del torneo<input required placeholder="Ej.: Copa Dominó de Octubre" value={formulario.nombre} onChange={evento => setFormulario({ ...formulario, nombre: evento.target.value })} /></label><label>Máximo de parejas<input type="number" min="4" max="30" step="2" value={formulario.max_parejas} onChange={evento => setFormulario({ ...formulario, max_parejas: evento.target.value })} /></label><label>Premio 1.º (fichas)<input type="number" min="0" value={formulario.premio_primero} onChange={evento => setFormulario({ ...formulario, premio_primero: evento.target.value })} /></label><label>Premio 2.º (fichas)<input type="number" min="0" value={formulario.premio_segundo} onChange={evento => setFormulario({ ...formulario, premio_segundo: evento.target.value })} /></label><label>Premio 3.º (fichas)<input type="number" min="0" value={formulario.premio_tercero} onChange={evento => setFormulario({ ...formulario, premio_tercero: evento.target.value })} /></label></div><button className="domino-primary" disabled={ocupado}>Crear torneo</button></form>}
