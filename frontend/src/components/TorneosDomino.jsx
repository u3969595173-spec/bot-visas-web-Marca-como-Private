...
function TorneosDomino({ admin = false }) {
  ...
  if (!seleccionado) return <main className="domino-page">
    <header className="domino-header"><div><p className="domino-eyebrow">Dominó suizo</p><h1>{admin ? 'Administrar torneos' : 'Torneos'}</h1><span>10 rondas suizas, top 8 y fase final.</span></div><Link to={admin ? '/admin' : '/domino'} className="domino-link">Volver</Link></header>
