...
function TorneosDomino({ admin = false }) {
  ...
  const simularBots = async () => {
    setOcupado(true); setError('Simulando una partida completa entre 4 bots...'); setSimulacion(null)
    try { setSimulacion(await request('/api/admin/domino/practica/simular', { method: 'POST', body: '{}' })) } catch (requestError) { setError(requestError.message) } finally { setOcupado(false) }
  }
  ...
  if (!seleccionado) return <main className="domino-page">
    ...
    {recursosTorneo}
    {admin && <><section className="domino-lobby-action"><h2>Partida de práctica</h2><button className="domino-secondary" type="button" onClick={simularBots} disabled={ocupado}>Jugar partida completa con 4 bots</button>{simulacion && <p><strong>{simulacion.ganador}</strong> ganó {simulacion.puntuacion?.[0]} - {simulacion.puntuacion?.[1]} en {simulacion.manos_jugadas} manos y {simulacion.turnos} turnos.</p>}</section><form className="domino-lobby-action domino-torneo-form" onSubmit={crearTorneo}><h2>Crear torneo</h2><div className="domino-torneo-fields"><label>Nombre del torneo<input required placeholder="Ej.: Copa Dominó de Octubre" value={formulario.nombre} onChange={evento => setFormulario({ ...formulario, nombre: evento.target.value })} /></label><label>Mínimo de parejas para iniciar<input type="number" min="4" max="30" step="2" value={formulario.min_parejas} onChange={evento => setFormulario({ ...formulario, min_parejas: evento.target.value })} /></label><label>Inscripción por pareja (fichas)<input type="number" min="0" value={formulario.costo_inscripcion} onChange={evento => setFormulario({ ...formulario, costo_inscripcion: evento.target.value })} /></label><label>Premio 1.º (fichas)<input type="number" min="0" value={formulario.premio_primero} onChange={evento => setFormulario({ ...formulario, premio_primero: evento.target.value })} /></label><label>Premio 2.º (fichas)<input type="number" min="0" value={formulario.premio_segundo} onChange={evento => setFormulario({ ...formulario, premio_segundo: evento.target.value })} /></label><label>Premio 3.º (fichas)<input type="number" min="0" value={formulario.premio_tercero} onChange={evento => setFormulario({ ...formulario, premio_tercero: evento.target.value })} /></label><label>Premio 4.º al 8.º (por pareja)<input type="number" min="0" value={formulario.premio_cuarto_a_octavo} onChange={evento => setFormulario({ ...formulario, premio_cuarto_a_octavo: evento.target.value })} /></label></div><button className="domino-primary" disabled={ocupado}>Crear torneo</button></form></>}
  ...
  return <main className="domino-page">
    ...
    {admin && <section className="domino-ubicacion"><span>Aprueba las parejas completas y genera la siguiente ronda cuando todas las mesas finalicen.</span><div className="domino-admin-actions"><button className="domino-secondary" type="button" onClick={simularBots} disabled={ocupado}>{ocupado ? 'Simulando partida...' : 'Jugar partida completa con 4 bots'}</button><button className="domino-primary" onClick={siguienteRonda} disabled={ocupado}>Generar siguiente ronda</button><button className="domino-delete" onClick={() => eliminarTorneo(seleccionado)} disabled={ocupado}>Eliminar torneo</button></div>{simulacion && <div><p><strong>Resultado: {simulacion.ganador}</strong> ganó {simulacion.puntuacion?.[0]} - {simulacion.puntuacion?.[1]} en {simulacion.manos_jugadas} manos y {simulacion.turnos} turnos.</p><p>{simulacion.eventos?.join(' ')}</p></div>}</section>}
