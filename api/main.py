...
async def registrar_estudiante(
    ...
    try:
        ...
        while True:
            check = db.execute(text("""
                SELECT id FROM estudiantes WHERE codigo_referido = :codigo
            """), {"codigo": codigo_referido}).fetchone()
            if not check:
                break
            codigo_referido = generar_codigo_acceso()
    ...
    try:
        ...
        cursor.execute("""
            UPDATE estudiantes SET
                ...
                nivel_espanol = %s,
                tipo_visa = %s,
                fondos_disponibles = COALESCE(%s, fondos_disponibles),
...
# ============================================================================
# CHAT - MARCAR COMO LEÍDO (Endpoints directos para arreglar problema)
# ============================================================================

@app.post("/api/estudiante/chat/{estudiante_id}/marcar-leidos", tags=["Chat"])
def estudiante_marcar_mensajes_leidos(
    ...
    try:
        from database.models import MensajeChat
...
@app.post("/api/admin/chat/{estudiante_id}/marcar-leidos", tags=["Chat"])
...
@app.post("/api/chat/{estudiante_id}/marcar-todos-leidos", tags=["Chat"])
...
def ver_tesoro_pagos(
    ...
    pagos_pendientes = []
    for row in result:
        # Las 3 modalidades son pagos que se hacen juntos, no opciones
        # El monto total es la suma de los 3
        precio_al_empezar = float(row[3]) if row[3] else 0
        precio_con_visa = float(row[4]) if row[4] else 0
        precio_financiado = float(row[5]) if row[5] else 0
        ...
        pagos_pendientes.append({
            ...
            'estudiante_id': row[1],
            'modalidad_seleccionada': 'paquete_completo',  # No es selección, es el paquete completo
...
def obtener_solicitudes_alojamiento(
    ...
    """Obtener todas las solicitudes de alojamiento de estudiantes"""
    try:
        result = db.execute(text("""
            SELECT 
                e.id,
