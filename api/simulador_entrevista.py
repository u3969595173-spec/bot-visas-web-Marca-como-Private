...
class SimuladorEntrevista:
    ...
    def generar_entrevista_personalizada(cls, estudiante_data: dict) -> dict:
        ...
        Retorna 10-15 preguntas adaptadas
        """
        ...
        if fondos < 8000:
            preguntas_seleccionadas.extend(cls.PREGUNTAS_FINANCIERAS[:2])
        else:
            preguntas_seleccionadas.extend(random.sample(cls.PREGUNTAS_FINANCIERAS, 1))
        
        # Preguntas sobre idioma si nivel es bajo
        if nivel_espanol in ['basico', 'ninguno']:
            preguntas_seleccionadas.append(cls.PREGUNTAS_ACADEMICAS[2])  # Nivel español
        
        # Preguntas personales
        preguntas_seleccionadas.extend(random.sample(cls.PREGUNTAS_PERSONALES, 2))
        
        # Preguntas críticas
        preguntas_seleccionadas.extend(cls.PREGUNTAS_CRITICAS)
